...
export async function normalizeTitle(title: string) {
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You are responsible for normalizing food titles so they can be matched with icons or generated as images when they don't exist. The input may be in any language, and the output should be a clear, simplified description of the food item. For example, "Quaker Cruesli cookies & cream" becomes "a box of cereal", while "KFC 32 Zinger Hot Wings bucket" is transformed into "a bucket of chicken". Keep descriptions singular—"fries" should become something like "a serving of fries" or "a packet of fries". Company-specific items like "Big Mac" should be generalized to "a hamburger"\n\nAdditionally, when the food is served in a container (such as a bowl, bucket, or plate) or is traditionally presented with culturally specific tableware, include that container in the description rendered in grey to ensure contrast with a white background. For example, if cereal is typically served in a bowl, the output should be "a grey bowl of cereal", and if the food is served in a bucket (as with KFC), it should be "a grey bucket filled with chicken". For items that are usually consumed without container details (such as a sport drink or a hamburger), simply describe the food item without adding container details.\n\nFor complex descriptors—such as "isometric sport drink"—simplify them by removing extraneous adjectives so that the description remains clear (for example, "a sport drink"). This approach keeps the description simple and ensures that any container traditionally associated with the food is rendered with consistent grey tableware.`,
      },
      {
        role: "user",
        content: [
